import type { Position } from '@/app/issue/types/idea';
...
export const calculateCategorySize = (itemCount: number) => {
  ...
  const width = 
    ...
    cols * IDEA_CARD.WIDTH + 
    (cols - 1) * CATEGORY_LAYOUT.GAP;
...
export const calculateGridPosition = (categoryPosition: Position, index: number, totalItems: number): Position => {
  ...
  const result = {
    ...
    y: categoryPosition.y + CATEGORY_LAYOUT.HEADER_HEIGHT + CATEGORY_LAYOUT.PADDING + row * (IDEA_CARD.HEIGHT + CATEGORY_LAYOUT.GAP),
  };  
  return result;
};